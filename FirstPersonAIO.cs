...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private void FixedUpdate()
    {
        ...
        if(advanced.FOVKickAmount>0){
            if(isSprinting && !isCrouching && playerCamera.fieldOfView != (baseCamFOV+(advanced.FOVKickAmount*2)-0.01f) && AutGun.isAiming == false){
            ...
            else if(playerCamera.fieldOfView != baseCamFOV && AutGun.isAiming == false){ playerCamera.fieldOfView = Mathf.SmoothDamp(playerCamera.fieldOfView,baseCamFOV,ref advanced.fovRef,advanced.changeTime*0.5f);}
    ...
    private void OnCollisionStay(Collision CollisionData) {
        if(IsGrounded || fps_Rigidbody.velocity.y < 0.1){
            IsGrounded = false;
            for(int i = 0; i< CollisionData.contacts.Count(); i++){
                if(CollisionData.contacts[i].point.y < (transform.position.y - (capsule.radius+0.199f))){
                    IsGrounded = true;
            ...
            for(int i = 0; i<CollisionData.contacts.Count(); i++){
                if(CollisionData.contacts[i].point.y < (transform.position.y - (capsule.radius+0.195f))){
                    if(advanced.maxSlopeAngle>0){
    ...
    private void OnCollisionExit(Collision CollisionData) {
        if(CollisionData.contacts.Count() == 0 && IsGrounded){IsGrounded = false;}
    }
}
    ...
    public class FPAIO_Editor : Editor{
        ...
        #region Movement Setup
            ...
            if(showAdvanced){
                ...
                if(t.advanced.FOVKickAmount > 0){t.advanced.changeTime = EditorGUILayout.Slider(new GUIContent("FOV Change Time","Determines the speed of the FOV kick"),t.advanced.changeTime,0.1f,5);}
                EditorGUILayout.Space();
        ...
        #region Audio/SFX Setup
            ...
            EditorGUILayout.Space();
            if(GUILayout.Button(adTex1, new GUIStyle(){stretchWidth = true,fixedWidth = Screen.width/1.11f,fixedHeight = 64, alignment = TextAnchor.MiddleCenter})){Application.OpenURL("https://assetstore.unity.com/packages/templates/systems/infinigun-universal-fps-firearm-153247");}
            EditorGUILayout.Space();
